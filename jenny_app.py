...
# # หน้า Log in
# def login_page():
#     # โค้ดสำหรับหน้าเข้าสู่ระบบ
#     st.title("Log in")
#     username = st.text_input("Enter your username:")
#     if st.button("Login"):
#         if username:  # ตรวจสอบว่าผู้ใช้กรอกชื่อแล้วหรือยัง
...
#                     # Update score
#                     st.session_state.score += 3.85  # Increment score by 3.85 for each A detected
...
def character_detection_mode():
    ...
    with col2:
        ...
        # Separate container for the "WELL DONE" message
        well_done_container = st.container()
        well_done_placeholder = well_done_container.empty()  # Placeholder for WELL DONE message
    ...
    if st.button('Start Detection'):
        ...
        log_df.to_csv('user_log.csv', index=False)  # Save to CSV file
        ...
        while st.session_state.run:
            ...
            if predicted_character == characters[st.session_state["char_index"]]:
                ...
                if elapsed_time >= 2:
                    ...
                    # Update score
                    st.session_state.score += 3.85  # Increment score by 3.85 for each detected character
